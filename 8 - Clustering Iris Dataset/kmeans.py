...
class KMeans(object):
    ...
    def initialize_centroids(self, data):
        ...
        for i in range(1, self.k):
            ...
            for j in range(len(self.centroids)):
                distance_from_centroid_j = self.get_euclidean_distance(sliced_data, self.centroids.iloc[j])
                distances = pd.concat([distances, distance_from_centroid_j], axis=1)
            ...
            # functions.
            min_distance_from_centroid = distances.min(axis=1)
            index = min_distance_from_centroid.idxmax() #index means getting index of the max distance point form nearest centroid
    ...
    def get_euclidean_distance(self, point1, point2):
        ...
        if type(point1) == pd.DataFrame:
            euclidean_d = np.sqrt(((point1 - point2)**2).sum(axis=1))
        else:
            euclidean_d = np.sqrt(((point1 - point2)**2).sum())
        return euclidean_d
    ...
    def group_observations(self, data):
        ...
        for i in range(self.k):
            distance_from_centroid_k = self.get_euclidean_distance(sliced_data, self.centroids.iloc[i])
            distances[str(i)] = distance_from_centroid_k
    ...
    def train(self, data, iters):
        ...
        while i < iters and not flag_groups and not flag_centroids:
            ...
            # data points.
            changes_cluster_group = np.where(groups != cur_groups, 1, 0).sum()
            flag_groups = (changes_cluster_group == 0)
            # TODO: Check if there are changes with the values of the centroids
            changes_value_centroid = (self.centroids-centroids)
            flag_centroids = (changes_value_centroid == 0).all(axis=None)
